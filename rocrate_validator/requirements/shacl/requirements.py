...
class SHACLRequirement(Requirement):
    ...
    def finalize(cls, context: ValidationContext) -> None:
        ...
        try:
            ...
            # The descriptor checks own the diagnostic when the metadata graph is
            # unavailable; a forced SHACL run cannot add useful information.
            logger.debug("Forced SHACL run skipped because metadata is unavailable: %s", e)
