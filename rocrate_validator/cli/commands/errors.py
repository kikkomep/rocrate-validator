...
def _print_debug_traceback(error: Exception, console: Console) -> None:
    ...
    # Rich's native traceback renderer assumes SyntaxError-compatible causes
    # always expose a non-null ``msg``. RDFLib's BadSyntax does not, so use the
    # standard formatter as input to Rich's syntax highlighter instead.
