...
from rocrate_validator.models import URI, Profile, Severity, ValidationContext, ValidationSettings
...
def local_jsonld_context(monkeypatch: pytest.MonkeyPatch) -> None:
    """Keep default-profile unit tests independent from remote contexts."""
    response = SimpleNamespace(
        status_code=HTTPStatus.OK,
        headers={"Content-Type": "application/ld+json"},
        json=lambda: {"@context": {}},
    )
    monkeypatch.setattr(HttpRequester, "get", lambda self, url, **kwargs: response, raising=False)
...
def test_default_profile_rejects_a_1_1_context() -> None:
    ...
    failed_check_ids = {issue.to_dict()["check"]["identifier"] for issue in result.get_issues()}
    assert "ro-crate-1.3_4.2" in failed_check_ids, (
        f"Expected the 1.3 context check to fail, got: {sorted(failed_check_ids)}"
    )

    # ...and the same context is accepted when explicitly pinned to 1.1.  This
    # assertion deliberately ignores unrelated SHACL results because this unit
    # test replaces the data graph to avoid resolving remote JSON-LD contexts.
    settings.profile_identifier = RO_CRATE_1_1_PROFILE_IDENTIFIER
    settings.skip_checks = ["ro-crate-1.1_4.1"]
    result = validate(settings)
    failed_check_ids = {issue.to_dict()["check"]["identifier"] for issue in result.get_issues()}
    assert "ro-crate-1.1_4.2" not in failed_check_ids
